...
class DaysOfMercyService
{
    use SendThankYouEmail;

    public string $startDate = '07-06-2024';
    public string $endDate = '16-06-2024';
...
//        try {
            ...
            $endDate = Carbon::createFromFormat('d-m-Y', $this->endDate, 'UTC')->midDay()->subHours(2)->timestamp;
...
//        }
    ...
    private function prepareDonation(float $amount, Order $order, string $ip, string $typeDonation, int $countryId): array
    {
        return [
            'value' => $amount,
            'order_id' => $order->id,
            'type' => Donation::TYPE_DAYS_OF_MERCY,
            'currency' => 'GBP',
    ...
    private function createPlans(Order $order, Customer $customer, array $donatesData, string $ip): Collection
    {
        ...
        foreach ($donatesData['amount'] as $key => $value) {
            ...
            if ((int)$donatesData['frequency'] === 3) {
                ...
                $countDay = Carbon::parse($this->endDate, 'UTC')->midDay()->diffInDays($startDate->midDay());
        ...
        if ((int)$donatesData['frequency'] === 2) {
            $this
                ->createSubscriptionTmp(
                    ...
                    Carbon::parse($this->endDate, 'UTC')->midDay()->subHours(2)->timestamp,
                    $plans,
                );
    ...
    {
        $days = Carbon::parse($this->endDate, 'UTC')->midDay()->diffInDays($startDate->midDay()) + 1;
    ...
    {
        if (Carbon::now()->gte(Carbon::parse($this->startDate)) && $frequency === 1) {
            return Carbon::now('UTC')->midDay();
        }

        if (Carbon::now()->lte(Carbon::parse($this->startDate)) && $frequency === 1) {
            return Carbon::parse($this->startDate, 'UTC')->midDay();
        }
