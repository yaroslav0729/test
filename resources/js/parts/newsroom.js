$(function() {
    ...
    function refreshTrendingArticles(response) {
        ...
        $(tab)
            .find("[newsroom-articles-body]")
            .html(newBody.html());

        let newPagination = $("[newsroom-articles-pagination]", response.html);
        $(tab)
            .find("[newsroom-articles-pagination]")
            .html(newPagination.html());
    }
    ...
    $(document).on("click", ".btn-newsroom", function() {
        ...
        let link = $(section)
            .find("[newsroom-articles] .pagination a")
            .eq(1);
        let path = $(link).attr("href");

        if (
            $(this).hasClass("btn-light-gray") ||
            $(this).hasClass("btn-white")
        ) {
        ...
        function changeStatusBtn(element) {
            ...
            //     className = "btn-white";
            //     secondChild = $(element)
            //         .closest(".newsroom-list")
            //         .find(".btn-white");
            // } else {
            className = "btn-primary-dark";
            secondChild = $(element)
                .closest("div")
                .find(".btn-primary-dark");
            // }

            if ($(parent).hasClass("newsroom-tab-white")) {
                newClassName = "btn-white";
            } else {
                newClassName = "btn-light-gray";
            }
            ...
            $(secondChild).addClass(newClassName);
