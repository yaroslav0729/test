...
if (!empty($keywordName) || !empty($keywordLocation)) {
    $query->where('name', 'LIKE', "%$keywordName%")->where('location', 'LIKE', "%$keywordLocation%");
}
...
$eventsForSlide =
    $events->count() < 3
        ? \App\Models\Event::getActualEventsQuery()
            ->take(3)
            ->get()
        : $events->take(3);
if ($eventsForSlide->isEmpty()) {
    $eventsForSlide = \App\Models\Event::getLastEvent()
        ->take(1)
        ->get();
}
...
@empty(!$eventsForSlide->count())
    ...
    <section class="events-home-swiper mt-5 no-loop" swiper-wrapper="events-slider">
        <div class="title">
            <p><b>OUR FEATURED EVENT</b></p>
        </div>
                        ...
                        <div class="swiper-slide">
                            <div class="row gutter-0">
                                ...
                                <div class="col-4 col-text text">
                                    <div>
                                        <a class="tl text-white text-decoration-none d-block"
                                            href="{{ url($event->page->getActualPageInstanceAttribute()->slug) }}">{{ $event->name }}</a>
                                        <svg class="decor-wave style-white mt-3" version="1.0"
    ...
    -16z" />
                                        ...
                                        <div class="pt-4"></div>
                                        <a class="d-block text-white event-description"
                                            href="{{ url($event->page->getActualPageInstanceAttribute()->slug) }}">{{ $event->page->getActualPageInstanceAttribute()->parameters['event_description'] ?? '' }}</a>
                                        <div class="place text-white"><i class="fal fa-map-marker-alt text-secondary"></i>
                                            {{ $event->location }}
                                        </div>
                                        <div>
            ...
            <div class="swiper next-only">
                <div class="swiper-button-prev"><i class="moon-icons-arrow-left"></i></div>
                <div class="swiper-button-next"><span>Next</span><i class="moon-icons-arrow-right"></i></div>
            </div>
        </div>
    </section>
    ...
    <section class="upcoming-events bg-light">
        <div class="wrap">
            <div class="row align-items-center upcoming-events-header">
                <div class="col-6">
                    <p class="font-size-30 pl-5 mt-n3"><b>Upcoming Events</b></p>
                </div>
                <div class="col-6 col-select text-right">
                        ...
                        <div class="form-group">
                            <select name="type" class="form-control filter" id="filter-type">
                                ...
                                @foreach (\App\Models\Event::ALL_TYPES_ENTRY as $typeId => $typeLabel)
                                    <option class="events-filter" value="{{ $typeId }}" @if ($typeId === intval($keywordType)) selected @endif>
                                        {{ $typeLabel }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            <select name="type" class="form-control filter" id="filter-participate">
                                ...
                                @foreach (\App\Models\Event::ALL_TYPE_EVENT as $typeId => $typeEvent)
                                    <option class="events-filter" value="{{ $typeId }}" @if ($typeId === intval($keywordTypeParticipate)) selected @endif>
                                        {{ $typeEvent }}</option>
                                @endforeach
                            </select>
                        </div>
