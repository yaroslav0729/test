        ...
        <div class="wrap">
            <div class="swiper-container">
                <div class="swiper-wrapper">
                    @for ($i = 1; $i <= 4; $i++)
                        @if (in_array($i, $hdrTypeActive)) @php
                        ...
                        class="learn-more text-underline
                        text-dark"><b>{{ $hdrLinkText[$i] }}</b></a>
                        </div>
                        <div class="title mb-4">
                        @if ($hdrColorType[$i] === 'blue')
                            {!! \App\Helpers\StrHelper::addSpanWithClass($hdrTitle[$i], 'text-info') !!}
                        @else
                            {!! \App\Helpers\StrHelper::addSpanWithClass($hdrTitle[$i], 'text-danger') !!} @endif
                </div>
                <p class="mb-5">{!! $hdrText[$i] !!}</p>
                <a href="{{ $donateLink[$i] }}" class="btn @if ($hdrColorType[$i]==='blue' ) btn-info @else btn-danger @endif">Donate
                    now</a>
            </div>
...
@include('modules.presentation.latest_projects', ["class" => 'homepage-latest-projects'])
