@extends('layouts.admin')

@section('content')
    
<div id="admin_content" class="bg-gray-100 flex-auto h-screen">
    <div class="p-5 pb-8">
        <h1>Campaign prices:</h1>

        @if (session('status'))
            <div class="alert alert-success" role="alert">
              <div class="flex">
                <div>
                  <p class="font-weight-bold">Success</p>
                  <p class="text-sm">{{ session('status') }}</p>
                </div>
              </div>
            </div>
        @endif

        <a href="{{ route('admin.campaign_prices.create') }}">
          <button class="btn btn-success mt-3 mb-3" type="button" title="Create campaign">
            <i class="far fa-plus-square mr-2"></i>Create
          </button>
        </a>

        <table class="table-auto mb-3">
            <thead>
              <tr>
                <th class="px-4 py-2">Id</th>
                <th class="px-4 py-2">Value</th>
                <th class="px-4 py-2">Type</th>
                <th class="px-4 py-2">Action</th>
              </tr>
            </thead>
            <tbody>

                @foreach ($campaignPrices as $price)
                    <tr>
                        <td class="border px-4 py-2">{{ $price->id }}</td>
                        <td class="border px-4 py-2">{{ $price->value }}</td>
                        <td class="border px-4 py-2">{{ $price->type_label }}</td>
                        <td class="border px-4 py-2 action_td">
                          <a href="{{ route('admin.campaign_prices.edit', ['campaign_price' => $price->id]) }}">
                            <button class="btn btn-info action-btn" type="submit" title="Edit campaign">
                              <i class="fas fa-edit"></i>
                            </button>
                          </a>
                          <form method="post" action="{{ route('admin.campaign_prices.destroy', ['campaign_price' => $price->id]) }}" style="display:inline-block">

                            @csrf
                            @method('DELETE')
    
                            <button class="btn btn-danger action-btn" type="submit" title="Delete campaign" onclick="return confirm('Are you sure want to delete?')">
                              <i class="fas fa-trash-alt"></i>
                            </button>
    
                        </form>
                        </td>
                    </tr>
                @endforeach

            </tbody>
          </table>
          {{ $campaignPrices->links() }}
    </div>
</div>

@endsection
